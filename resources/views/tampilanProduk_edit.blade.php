...
@section('content')
    <div class="form-container">
        <h2 class="text-2xl font-bold mb-6">Formulir Edit Produk: {{ $product->name }}</h2>

        {{-- INI ADALAH BARIS YANG SUDAH DIPERBAIKI TOTAL --}}
        <form action="{{ route('admin.produk.update', $products->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="category" id="category" class="form-control" value="{{ $products->category }}"
                    required>
            </div>

            <div class="form-group">
                <label for="icon" class="form-label">Ikon (Emoji)</label>
                <input type="text" name="icon" id="icon" class="form-control" value="{{ $products->icon }}">
            </div>
